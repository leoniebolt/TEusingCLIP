...
# config
bag_path = Path(
    "datasets/robonav/mattro_route1/mattro_01_2022-06-23-07-29-23.bag"
)
output_dir = Path("datasets/robonav/mattro_route1")
csv_dir = output_dir / "csv_files"
csv_dir.mkdir(parents=True, exist_ok=True)

image_dir_left = output_dir / "images/original/left"
image_dir_left.mkdir(parents=True, exist_ok=True)

image_dir_right = output_dir / "images/original/right"
  ...
  for conn, timestamp, rawdata in reader.messages():
    ...
    if conn.topic == image_topic_left:
      ...
      timestamps_left.append(
          {"filename": filename, "timestamp_sec": timestamp_sec}
      )
...
camera_left_df.to_csv(csv_dir / "camera_left.csv", index=False)
camera_right_df.to_csv(csv_dir / "camera_right.csv", index=False)

left_df.to_csv(csv_dir / "timestamps_left.csv", index=False)
right_df.to_csv(csv_dir / "timestamps_right.csv", index=False)

# summary
print("Left images:", len(left_df))
print("Right images:", len(right_df))
